...
seq_length = 10 # window
secs_for_action = 10 # action recording time
...
#                angle_label = np.append(angle_label, idx) # labeling 0, 1, 2
 ...
 #              d = np.concatenate([face_point.flatten(), hand_joint.flatten(), angle_label])   # face_point 2차원 array, hand_joint 2차원 array, 라벨링 1차원 array로 만들고 concatenate
        ...
        np.save(os.path.join('dataset', f'vx_{action}'), vx)
        np.save(os.path.join('dataset', f'vy_{action}'), vy)
        np.save(os.path.join('dataset', f'vz_{action}'), vz)
